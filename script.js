...
function gameLoop() {
  ...
  function modifyScore(computer, player) {
    if (playerScore === 5 || computerScore === 5) {
      const gameWin = prompt(
        `Someone won! Please answer "yes" if you would like to play again, or "no" if not.`
      );
      if (gameWin.toLowerCase() === "yes") {
        gameLoop();
      } else {
      ...
      console.log(
        `Computer score is ${computerScore} and player score is ${playerScore}.`
      );
      ...
      if (computer === "Rock" && player === "Scissors") {
        ...
        console.log(
          `The computer chose ${computer} and you chose ${player}. ${computer} is the same as ${player}, no points were awarded.`
        );
